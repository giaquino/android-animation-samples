...
public class PasswordEntry extends TextInputEditText {
   ...
   */
  @TargetApi(Build.VERSION_CODES.LOLLIPOP) private static class MaskMorphDrawable extends Drawable {
    ...
    MaskMorphDrawable(Context context, TextPaint textPaint, int baseline, float charWidth,
        int insetStart) {
      ...
      showPasswordDuration = 200;
      hidePasswordDuration = 320;
      fastOutSlowIn =
    ...
    private Animator morphPassword(CharSequence pw, float fromProgress, float toProgress,
        long duration) {
      password = pw;
      updateBounds();
  ...
  static class PasswordCharacter {
    ...
    PasswordCharacter(String password, int index, TextPaint paint, float maskCharDiameter,
        float maskCenterY) {
    ...
    void draw(Canvas canvas, TextPaint paint, CharSequence password, int index, float charWidth,
      ...
      canvas.drawText(password, index, index + 1, x, lerp(0f, textOffsetY, progress) / textScale,
          paint);
      canvas.restore();
